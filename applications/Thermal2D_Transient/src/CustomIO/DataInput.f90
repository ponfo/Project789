...
contains
  ...
  subroutine initMaterials(thermalAppl)
    ...
    real(rkind) :: kx, ky, cp, rho
    do i = 1, 7
       read(project,*)
    end do
    if(verbose) print'(A)', 'Material         Kx            Ky           cp          rho       '
    do i = 1, nMaterial
       read(project,*) iMat, kx, ky, cp, rho
       thermalAppl%material(iMat) = thermalMaterial(kx, ky, cp, rho)
       if(verbose) print'(4X,I0,7X,2(E10.3,3X))', iMat, kx, ky, cp, rho 
    end do
