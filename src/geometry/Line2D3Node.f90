module Line2D3NodeM
  ...
  private
  public :: Line2D3NodeDT, line2D3Node

  type, extends(GeometryDT) :: Line2D3NodeDT
   contains
  ...
  end type Line2D3NodeDT

  interface line2D3Node
     procedure :: constructor
  end interface line2D3Node

  integer(ikind), parameter :: NNODE = 3

contains

  type(Line2D3NodeDT) function constructor(gaussOrder)
    implicit none
  ...
  real(rkind) function jacobianDetFromCoordAllNodes(this, pointToValue, node)
    ...
    jacobianDetFromCoordAllNodes = sqrt(jacobian(1,1)**2 + jacobian(1,2)**2)
  ...
  real(rkind) function jacobianDetFromCoordSomeNodes(this, indexList, pointToValue, node)
    ...
    jacobianDetFromCoordSomeNodes = sqrt(jacobian(1,1)**2 + jacobian(1,2)**2)
  ...
  real(rkind) function jacobianDetFromJacobian(this, jacobian)
    ...
    jacobianDetFromJacobian = sqrt(jacobian(1,1)**2 + jacobian(1,2)**2)
  ...
  function jacobianDetAtGPointsFromCoord(this, node)
    ...
    do i = 1, this%integrator%integTerms
       jacobianDetAtGPointsFromCoord(i) = sqrt(jacobian(i,1,1)**2 + jacobian(i,1,2)**2)
    end do
  ...
  function jacobianDetAtGPointsFromJacobian(this, jacobian)
    ...
    integer(ikind)                                     :: i
    do i = 1, this%integrator%integTerms
       jacobianDetAtGPointsFromJacobian(i) = sqrt(jacobian(i,1,1)**2 + jacobian(i,1,2)**2)
    end do
