module Triangle3D6NodeM
  ...
  private
  public :: Triangle3D6NodeDT, triangle3D6Node

  type, extends(GeometryDT) :: Triangle3D6NodeDT
   contains
  ...
  end type Triangle3D6NodeDT

  interface triangle3D6Node
     procedure :: constructor
  end interface triangle3D6Node

  integer(ikind), parameter :: NNODE = 6

contains

  type(Triangle3D6NodeDT) function constructor(gaussOrder)
    implicit none
  ...
  subroutine init(this, gaussOrder)
    implicit none
    class(Triangle3D6NodeDT), intent(inout) :: this
    integer(ikind)          , intent(in)    :: gaussOrder
    this%nNode = NNODE
    this%dim = 3
