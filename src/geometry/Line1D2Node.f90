module Line1D2NodeM
  ...
  private
  public :: Line1D2NodeDT, line1D2Node

  type, extends(GeometryDT) :: Line1D2NodeDT
   contains
  ...
  end type Line1D2NodeDT

  interface line1D2Node
     procedure :: constructor
  end interface line1D2Node

  integer(ikind), parameter :: NNODE = 2

contains

  type(Line1D2NodeDT) function constructor(gaussOrder)
    implicit none
  ...
  subroutine init(this, gaussOrder)
    implicit none
    class(Line1D2NodeDT), intent(inout) :: this
    integer(ikind)      , intent(in)    :: gaussOrder
    this%nNode = NNODE
    this%dim = 1
